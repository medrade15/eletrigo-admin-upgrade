...
import { updateElectrician, createProduct, updateProduct, deleteProduct } from '../api';
...
const ElectriciansPage = ({electricians, setElectricians}: {electricians: Electrician[], setElectricians: React.Dispatch<React.SetStateAction<Electrician[]>>}) => {
    ...
    const handleStatusChange = async (id: string, status: ElectricianStatus) => {
        ...
        try {
            await updateElectrician(id, { status });
        } catch (err) {
            alert('Falha ao atualizar status no servidor. Recarregue a página.');
        }
    };
...
const ProductsPage = ({initialProducts}: {initialProducts: Product[]}) => {
    ...
    const handleSaveProduct = async (productData: Product) => {
        ...
        try {
            if (isEditing) {
                const updated = await updateProduct(productData.id, {
                ...
                setProducts(products.map(p => p.id === productData.id ? (updated as any) : p));
            } else {
                ...
                setProducts([...products, created as any]);
            }
            handleCloseModal();
        } catch (err) {
            alert('Falha ao salvar produto no servidor. Tente novamente.');
        }
    };
