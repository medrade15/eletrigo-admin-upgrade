...
import { 
  getElectricians, getClients, getProducts, getServices,
  createElectrician, createClient,
  createService, updateService, updateClient,
  adminLogin
} from './api';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const [els, cls, prods, svcs] = await Promise.all([
                    getElectricians(),
                    getClients(),
                    getProducts(),
                    getServices()
                ]);
                if (els && els.length >= 0) setElectricians(els);
                if (cls && cls.length >= 0) setClients(cls);
                if (prods && prods.length >= 0) setProducts(prods);
                if (svcs && svcs.length >= 0) setServices(svcs);
                addNotification('Dados carregados do servidor.');
            } catch (err) {
                console.error(err);
                addNotification('Falha ao carregar dados do servidor. Usando dados locais.');
            }
        };
        load();
    ...
    const handleElectricianRegister = useCallback(async (newElectrician: Electrician) => {
        try {
            const created = await createElectrician({
                name: newElectrician.name,
                cpf: newElectrician.cpf,
                phone: newElectrician.phone,
                email: newElectrician.email,
                address: newElectrician.address,
                experience: newElectrician.experience,
                profilePictureUrl: newElectrician.profilePictureUrl,
                documentUrl: newElectrician.documentUrl,
                status: newElectrician.status,
                rating: newElectrician.rating,
                joinDate: newElectrician.joinDate,
            });
            setElectricians(prev => [...prev, created as any]);
            addNotification('Cadastro realizado! Aguarde aprovação do administrador.');
        } catch (err) {
            console.error(err);
            addNotification('Erro ao cadastrar eletricista.');
        }
    ...
    const handleClientRegister = useCallback(async (newClient: Client) => {
        try {
            ...
            setCurrentClient(created as any);
            setView('client_dashboard');
            addNotification('Cadastro realizado com sucesso!');
        } catch (err) {
            console.error(err);
            addNotification('Erro ao cadastrar cliente.');
        }
    }, [addNotification]);
    ...
    const handleUpdateService = useCallback(async (id: string, status: ServiceStatus) => {
        try {
            const updated = await updateService(id, { status });
            setServices(prev => prev.map(s => s.id === id ? { ...s, status: (updated as any).status } : s));
            ...
            console.error(err);
            addNotification('Erro ao atualizar serviço.');
        }
    }, [addNotification, services]);

    const handleAcceptService = useCallback(async (serviceId: string, electricianId: string, electricianName: string, eta: number) => {
        try {
            const updated = await updateService(serviceId, { status: ServiceStatus.Accepted, electricianId, electricianName, eta });
            setServices(prev => prev.map(s => s.id === serviceId ? { ...s, status: ServiceStatus.Accepted, electricianId, electricianName, eta } : s));
            addNotification(`Seu serviço foi aceito! ${electricianName} está a caminho.`);
        } catch (err) {
            console.error(err);
            addNotification('Erro ao aceitar serviço.');
        }
    }, [addNotification]);
    
    const handleRequestService = useCallback(async (serviceData: Omit<Service, 'id' | 'value'>) => {
        const payload = {
            ...serviceData,
            status: ServiceStatus.Requested,
            date: new Date().toISOString().split('T')[0],
            value: serviceData.serviceType === 'Emergencial'
                ? Math.floor(Math.random() * 100) + 150
                : Math.floor(Math.random() * 150) + 200,
        };
        try {
            const created = await createService(payload as any);
            setServices(prev => [created as any, ...prev]);
            ...
            console.error(err);
            addNotification('Erro ao solicitar serviço.');
        }
    }, [addNotification]);

    const handleCancelService = useCallback(async (serviceId: string) => {
        try {
            const updated = await updateService(serviceId, { status: ServiceStatus.Cancelled });
            setServices(prev => prev.map(s => s.id === serviceId ? { ...s, status: (updated as any).status } : s));
        } catch (err) {
            console.error(err);
            addNotification('Erro ao cancelar serviço.');
        }
    }, [addNotification]);

    const handleSendMessage = useCallback(async (serviceId: string, sender: 'client' | 'electrician', message: string) => {
        const timestamp = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
        let nextChat: ChatMessage[] | undefined;
        setServices(prev => prev.map(s => {
            if (s.id === serviceId) {
                const newMessage: ChatMessage = { sender, message, timestamp };
                nextChat = s.chat ? [...s.chat, newMessage] : [newMessage];
        ...
        try {
            if (nextChat) await updateService(serviceId, { chat: nextChat });
        } catch (err) {
            console.error(err);
            addNotification('Falha ao registrar mensagem no servidor.');
        }
    ...
    const handleUpdateClientProfile = useCallback(async (clientId: string, profileData: Omit<Client, 'id' | 'joinDate'>) => {
        try {
            const updated = await updateClient(clientId, profileData as any);
            setClients(prev => prev.map(c => c.id === clientId ? (updated as any) : c));
            setCurrentClient(updated as any);
